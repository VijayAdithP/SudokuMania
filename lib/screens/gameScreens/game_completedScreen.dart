...
class _GameCompletedscreenState extends ConsumerState<GameCompletedscreen> {
  ...
  Future<void> _saveScore() async {
    ...
    // This is the score for daily challenge
    ...
    log("Before updating this is the prevscore ${currentStats.totalPoints}");
    log("Before updating this is the to be added score $score");
    log("Before updating this is the current score ${currentStats.totalPoints + score}");
    switch (difficulty) {
      case 'Easy':
        currentStats = currentStats.copyWith(
          totalPoints: currentStats.totalPoints + score,
    ...
    await HiveService.saveUserStats(currentStats);
    UserStats? totest = await HiveService.loadUserStats();
    log("this is the prevscore ${totest!.totalPoints}");
    log("this is the to be added score $score");
    log("this is the current score ${totest.totalPoints + score}");
  }

  Future<void> _updateGameStreaksAndWonCount() async {
    UserStats currentStats = await HiveService.loadUserStats() ?? UserStats();
  ...
  Future<void> _updateWinRate() async {
    UserStats currentStats = await HiveService.loadUserStats() ?? UserStats();
    currentStats = currentStats.copyWith(
      easyWinRate: UserStats.calculateWinRate(
          currentStats.easyGamesWon, currentStats.easyGamesStarted),
  ...
  Future<void> _updateBestTime() async {
    ...
    // Handle the case where validTimes is empty
    int? minTime;
    if (validTimes.isNotEmpty) {
      minTime = validTimes.reduce((a, b) => a! < b! ? a : b);
    } else {
      minTime = null; // or set a default value like 0
    }
